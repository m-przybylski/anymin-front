...
export class PrecallModalController implements ng.IController {
  ...
  private PaymentsGetCreditCardsRoute = (): void => {
    this.PaymentsApi.getCreditCardsRoute().then(
      paymentMethods => {
  ...
  private FinanceGetClientBalanceRoute = (): void => {
    this.FinancesApi.getClientBalanceRoute().then(
      clientBalance => {
        this.prepaidCallLimitModel = clientBalance.accountBalance.amount;
        this.clientBalance = {
          name:
            this.prepaidTranslation +
            ' ' +
            (clientBalance.accountBalance.amount / this.moneyDivider).toFixed(this.moneyNumberOfDecimalPlaces) +
            ' ' +
            clientBalance.accountBalance.currency,
