...
import { LoggerService, CommunicatorService, MessageRoom, CurrentCall } from '@anymind-ng/core';
...
export class MessengerMaximizedComponentController
  implements ng.IController, IMessengerMaximizedComponentBindings, ng.IOnDestroy {
  ...
  public uploadAgain = (): void => this.onUploadFiles(this.uploadedFile.file ? [this.uploadedFile.file] : []);
  ...
  private updateChatHistory = (messageRoom: MessageRoom): void => {
    this.isLoading = true;
    messageRoom.getHistory(0, this.historyLimit).then(this.handleRoomHistory, err => {
  ...
  private expertInit = (expertSessionCall: IExpertSessionCall): void => {
    this.participantAvatar = '';
    this.onNewCall(expertSessionCall.currentExpertCall);
  };
  ...
  private sendMessage = (messageObject: string, context: IMessageContext): void => {
    this.messageRoom
      ? this.messageRoom.sendMessage(messageObject, context).catch(this.onMessageSendError)
  ...
  private uploadFile = (file: File): ng.IPromise<void> =>
    this.uploader
      .uploadFile(file, this.postProcessOptions, this.onUploadProgess)
