...
export class BraintreeFormComponentController implements ng.IController, IBraintreeFormComponentBindings {
            ...
            (err, hostedFieldsInstance) => {
              ...
              hostedFieldsInstance.on('cardTypeChange', (event: any) => {
                if (event.cards.length === 1) {
                  angular.element('#card-activityType').text(event.cards[0].niceType);
                } else {
                  angular.element('#card-activityType').text('');
                }
              });
  ...
  public checkIsValueCorrect = (): boolean => !!this.defaultCardLimit && this.showCardLimitForm;
}
