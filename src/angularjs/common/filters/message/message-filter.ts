...
function messageFilter(
...
): (message: roomEvents.CustomMessageSent) => string {
  ...
  const handleImageMessage = (message: roomEvents.CustomMessageSent): string => {
    ...
    return `<a href="${fileUrl}" target="_blank">` + `<img src="${fileUrl}"/></a>`;
  };
      ...
      for (const url in messageUrls) {
        if (messageUrls.hasOwnProperty(url)) {
          ...
          if (hasImageUrl(currentUrl)) {
            return message.context.content.replace(
              urlRegexp,
              '<a href="' +
                getCorrectUrl(currentUrl) +
                '" target="_blank" >' +
                '<img src="' +
                getCorrectUrl(currentUrl) +
                '"/></a>',
            ...
            return message.context.content.replace(
              urlRegexp,
              '<a href="' + getCorrectUrl(currentUrl) + '" target="_blank">' + currentUrl + '</a>',
            );
          }
        }
      }
...
angular.module('profitelo.filters.message-filter', []).filter('message', ['urlService', 'logger', messageFilter]);
