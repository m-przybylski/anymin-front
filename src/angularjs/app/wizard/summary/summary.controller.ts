...
export class SummaryController implements ng.IController {
  ...
  public $onInit(): void {
    this.isConsultation =
      (this.wizardProfile.services && this.wizardProfile.services.length > 0) || this.isConsultationInvitationAccepted;
  ...
  public saveWizard = (): void => {
    if (this.checkIsWizardValid()) {
      this.WizardApi.postWizardCompleteRoute().then(
        _response => {
          this.userService.getUser(true).then(user => {
            if (this.checkIsWizardHasInvitationServices()) {
              this.acceptInvitations()
                ...
                .finally(() => this.redirectToUserSetting(user.id));
            } else {
              this.redirectToUserSetting(user.id);
            }
          });
        },
        error => {
  ...
  private redirectToUserSetting = (id: string): void => {
    this.$state.go(`app.dashboard.expert-view.${id}`);
  };
  ...
  private setInvitationsServices = (): void => {
    ...
    if (acceptedConsultationsObject) {
      this.InvitationApi.getInvitationsRoute().then(
        invitations => {
          const differenceArray = _.difference(
            JSON.parse(acceptedConsultationsObject).map(
              (accpetedConsultation: IGetServiceWithInvitationsAndTags) => accpetedConsultation.invitation.id,
            ),
  ...
  private checkIsWizardValid = (): boolean => {
    if (this.wizardProfile.isSummary && (this.checkIsWizardHasService() || this.checkIsWizardHasInvitationServices())) {
      if (!this.wizardProfile.isCompany && this.checkIsExpertProfileValid()) {
        ...
        this.wizardProfile.isCompany &&
        this.wizardProfile.isExpert &&
        this.checkIsExpertProfileValid() &&
  ...
  private checkIsExpertNameValid = (): boolean =>
    this.wizardProfile.expertDetailsOption && this.wizardProfile.expertDetailsOption.name
      ? this.namePattern.test(this.wizardProfile.expertDetailsOption.name)
      : false;

  private checkIsExpertDescriptionValid = (): boolean =>
    this.wizardProfile.expertDetailsOption && this.wizardProfile.expertDetailsOption.description
      ? this.descriptionPattern.test(this.wizardProfile.expertDetailsOption.description)
      : false;

  private checkIsExpertProfileValid = (): string | boolean | undefined =>
    this.wizardProfile.isExpert &&
    this.wizardProfile.expertDetailsOption &&
    this.wizardProfile.expertDetailsOption.avatar &&
    this.checkIsExpertDescriptionValid() &&
  ...
  private checkIsCompanyNameValid = (): boolean =>
    this.wizardProfile.organizationDetailsOption && this.wizardProfile.organizationDetailsOption.name
      ? this.namePattern.test(this.wizardProfile.organizationDetailsOption.name)
      : false;

  private checkIsCompanyDescriptionValid = (): boolean =>
    this.wizardProfile.organizationDetailsOption && this.wizardProfile.organizationDetailsOption.description
      ? this.descriptionPattern.test(this.wizardProfile.organizationDetailsOption.description)
      : false;

  private checkIsCompanyProfileValid = (): string | boolean | undefined =>
    this.wizardProfile.isCompany &&
    this.wizardProfile.organizationDetailsOption &&
    this.checkIsCompanyNameValid() &&
    this.wizardProfile.organizationDetailsOption.logo &&
  ...
  private checkIfUserCanCreateExpertProfile = (): boolean => {
    if (this.wizardProfile && this.wizardProfile.services && !this.acceptedServices) {
      return !!(
        _.find(this.wizardProfile.services, service => service.isOwnerEmployee) && !this.wizardProfile.isExpert
      );
  ...
  private acceptInvitations = (): ng.IPromise<void> =>
    this.$q
      .all(
        this.acceptedServices.map(acceptedService =>
          this.InvitationApi.postInvitationAcceptRoute(acceptedService.invitation.id),
        ),
      )
