...
import { MoneyDto, GetCompanyInvoiceDetails, GetCreditCard } from 'profitelo-api-ng/model/models';
...
export class DashboardSettingsPaymentsController implements ng.IController {
  ...
  constructor(
    getInvoiceData: void | GetCompanyInvoiceDetails,
    ...
    if (getInvoiceData) {
      ...
      if (getInvoiceData.address) {
        this.address =
          getInvoiceData.address.address +
          ', ' +
          getInvoiceData.address.postalCode +
          ', ' +
          getInvoiceData.address.city +
          ', ' +
    ...
    FinancesApi.getClientBalanceRoute()
      .then(
        clientBalance => {
          this.accountBalance = clientBalance.accountBalance;
          if (clientBalance.accountBalance.amount !== 0) this.isAnyPaymentMethod = true;
        },
        error => {
    ...
    PaymentsApi.getCreditCardsRoute()
      .then(
        paymentMethods => {
          ...
          PaymentsApi.getDefaultPaymentMethodRoute().then(
            response => {
              if (response.card) {
...
const paymentsSettingsModule = angular
  .module('profitelo.controller.dashboard.settings.payments', [
    apiModule,
  ...
  .controller('dashboardSettingsPaymentsController', DashboardSettingsPaymentsController).name;
