...
import { GetServiceWithInvitations, GetInvitation } from 'profitelo-api-ng/model/models';
...
export class DashboardExpertEmployeesController {
  ...
  private postServiceWithInvitations = (servicesId: string[]): void => {
    ...
    this.ServiceApi.postServiceInvitationsRoute({ serviceIds: servicesId })
      .then(servicesWithInvitations => {
  ...
  private groupInvitations = (servicesWithInvitations: GetServiceWithInvitations[]): void => {
    const invitationsByEmail = _.flatMap(servicesWithInvitations, service =>
      service.invitations.filter(
        invitation => invitation.status === GetInvitation.StatusEnum.NEW && invitation.email !== undefined,
      ),
    );
    ...
    const invitationsByMsisdn = _.flatMap(servicesWithInvitations, service =>
      service.invitations.filter(
        invitation => invitation.status === GetInvitation.StatusEnum.NEW && invitation.msisdn !== undefined,
      ),
    );
