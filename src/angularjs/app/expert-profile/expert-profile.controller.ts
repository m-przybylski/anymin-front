...
import { ExpertProfileView, ServiceWithOwnerProfile } from '@anymind-ng/api';
import { ExpertProfileWithDocuments } from '@anymind-ng/api/model/expertProfileWithDocuments';
...
export class ExpertProfileController {
  ...
  constructor(
    private $stateParams: IExpertProfileStateParams,
  ...
  public onProfileLike = (): boolean => (this.isFavourite = true);

  public onProfileLikeError = (error: any): void =>
    this.$log.error('Can not like this company because: ' + String(error));

  public onProfileDislike = (): boolean => (this.isFavourite = false);
  ...
  public handleLike = (): void => {
    if (!this.isFavourite) {
      this.ProfileApi.postProfileFavouriteExpertRoute(this.$stateParams.profileId).then(
        this.onProfileLike,
        this.onProfileLikeError,
      );
    } else {
      this.ProfileApi.deleteProfileFavouriteExpertRoute(this.$stateParams.profileId).then(
        this.onProfileDislike,
        this.onProfileDislikeError,
      );
    }
  };
}
