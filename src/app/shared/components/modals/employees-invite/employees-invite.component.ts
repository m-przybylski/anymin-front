...
export class EmployeesInviteModalComponent implements OnInit, AfterViewInit {
  ...
  public ngOnInit(): void {
    this.employeesInviteService.getEmployeeList()
      ...
      .subscribe((response: ReadonlyArray<ExpertProfileWithEmployments>) => {
        ...
        this.employeesConsultationList =
          this.dropdownItems.filter(employee => employee.serviceId === this.serviceId);
        this.employeesConsultationList.forEach(item => {
          this.filterOwnEmployeesDropdownList(item);
        });
      });

    this.employeesInviteService.getConsultationDetails(this.serviceId)
      .pipe(takeUntil(this.ngUnsubscribe$))
  ...
  public onClickSend = (formGroup: FormGroup): void => {
    if (this.invitedEmployeeList.length !== 0 && formGroup.valid) {
      this.employeesInviteService.postInvitation({invitations: this.adjustEmployeeInvitationObject()})
        .pipe(takeUntil(this.ngUnsubscribe$))
  ...
  private filterItem = (value: string): IEmployeesInviteComponent[] =>
    this.filteredItems = this.dropdownItems.filter(
  ...
  private handleGetEmployeeListError = (error: HttpErrorResponse, msg: string):
