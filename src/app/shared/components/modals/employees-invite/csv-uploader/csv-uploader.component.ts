...
import { IValidatorsErrorMsg } from '../../../inputs/input-add-item/input-add-item.component';
...
export class CsvUploaderComponent implements OnInit {
  ...
  public onLoadFile = (csv: ReadonlyArray<string>): void => {
    const emailsOrMsisdns = csv.reduce((a, b) => a.concat(b), [] as ReadonlyArray<string>)
      .filter(str => str.length > 0).map(str => str.replace(/\s/g, ''));

    this.uploadedFilesLength = 0;
    this.incorrectInvitations = 0;

    emailsOrMsisdns.forEach((emailOrMsisdn): void => {
  ...
  private isEmailValid = (emailOrMsisdn: string): boolean =>
    this.addedItemsList.indexOf(emailOrMsisdn) === -1 && !this.isValueExist(emailOrMsisdn) &&
    !this.areInvitationsExceedValidLimit()

  private isMsisdn = (arrayNumber: string): boolean =>
    isValidNumber(arrayNumber) && this.isMsisdnValid(arrayNumber)

  private isMsisdnValid = (arrayNumber: string): boolean =>
    this.addedItemsList.indexOf(arrayNumber) === -1 && !this.isValueExist(arrayNumber) &&
    !this.areInvitationsExceedValidLimit()
