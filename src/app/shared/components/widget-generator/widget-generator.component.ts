...
import { IPrimaryDropdownListElement } from '../../../../angularjs/common/components/interface/dropdown-primary/dropdown-primary';
import { WidgetGeneratorService } from './widget-generator.service';
import { UserSessionService } from '../../../core/services/user-session/user-session.service';
import { ServiceWithEmployments, GetProfile } from '@anymind-ng/api';
...
export class WidgetGeneratorComponent implements OnInit {
  ...
  public selectRadio = (value: string): void => {
    ...
    if (typeof this.bodyScript !== 'undefined') {
  ...
  public generateWidgetCode = (expertId?: string, serviceId?: string): void => {
    ...
    this.clearButtonClicked();
    this.widgetGeneratorService.generateWidget(expertId, serviceId).subscribe(
      widget => {
        this.isError = false;
  ...
  private getExpertInitializeData = (accountId: string): void => {
    this.widgetGeneratorService.getExpertProfileWithServices(accountId).then(
      profileWithServices => {
        this.expertId = profileWithServices.expertProfile.id;
        this.serviceList = profileWithServices.employments
          .filter(employment => typeof employment.serviceDetails.deletedAt !== 'undefined')
          .filter(
            serviceWithProfile =>
              typeof (serviceWithProfile.serviceDetails.ownerProfile as GetProfile | undefined) !== 'undefined' &&
  ...
  private getCompanyInitializeData = (accountId: string): void => {
    this.widgetGeneratorService.getOrganizationProfilesWithServices(accountId).then(
      profileWithServices => {
        this.expertList = _.uniqBy(
          _.flatMap(profileWithServices.services, profileWithSvc =>
            profileWithSvc.employments.map(employment => ({
        ...
        this.serviceList = profileWithServices.services
          .filter(
            profileWithSvc =>
              typeof profileWithSvc.service.deletedAt !== 'undefined' && profileWithSvc.employments.length > 0,
          )
          .map(profileWithSvc => ({
            name: profileWithSvc.service.name,
            value: profileWithSvc.service.id,
          }));
        this.isUserHasAnyServices = this.serviceList.length > 0;
        ...
        this.profileWithServices = profileWithServices.services.filter(
          profileWithSvc => typeof profileWithSvc.service.deletedAt !== 'undefined',
        );
        this.isLoading = false;
      },
      error => {
  ...
  private generateBodyCode = (widgetId?: string): string =>
    ...
    `<button data-anymind-widget="${widgetId}" class="anymind-button${
      this.radioModel === 'flatten' ? ' anymind-floating' : ''
    }"></button>`;

  private reloadServicesDropdown = (): void => {
    this.serviceCompanyList = this.profileWithServices
      .filter(service => _.find(service.employments, employment => employment.employeeProfile.id === this.expertId))
      .map(serviceWithEmployees => ({
  ...
  private setDropdownSelectedService = (): void => {
    if (typeof _.find(this.serviceCompanyList, service => service.value === this.serviceId) !== 'undefined') {
  ...
  private checkIsServiceWidgetFormValid = (): boolean =>
    this.widgetTypeModel === 'service' &&
    !(!this.isWidgetGenerating && (typeof this.expertId !== 'undefined' || typeof this.serviceId !== 'undefined'));

  private checkIsExpertWidgetFormValid = (): boolean =>
    this.widgetTypeModel === 'expert' &&
    !(!this.isWidgetGenerating && (typeof this.expertId !== 'undefined' && typeof this.serviceId !== 'undefined'));
}
