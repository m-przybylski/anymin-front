    ...
    this.widgetGeneratorService.getOrganizationProfilesWithServices(accountId).then(
      profileWithServices => {
        this.expertList = _.uniqBy(
          _.flatMap(profileWithServices.services, profileWithServices =>
            profileWithServices.employments.map(employment => ({
        ...
        this.serviceList = profileWithServices.services
          .filter(
            profileWithServices => !profileWithServices.service.deletedAt && profileWithServices.employments.length > 0,
          )
        ...
        this.profileWithServices = profileWithServices.services.filter(
          profileWithServices => !profileWithServices.service.deletedAt,
        );
        this.isLoading = false;
      },
      error => {
