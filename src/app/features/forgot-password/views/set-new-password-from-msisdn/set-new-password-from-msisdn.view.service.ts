...
export class SetNewPasswordFromMsisdnViewService {
  ...
  public handleNewPassword =
    (msisdn: string, token: string, password: string): Observable<SetNewPasswordFromMsisdnStatus> =>
      this.recoverPasswordService.putRecoverPasswordMsisdnRoute({msisdn, token, password})
        .pipe(mergeMap(() => fromPromise(this.login(msisdn, password))))
  ...
  private handlePutRecoverPasswordMsisdnRoute =
    (httpError: HttpErrorResponse): Observable<SetNewPasswordFromMsisdnStatus> => {
