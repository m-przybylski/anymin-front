  ...
  private redirect = (methodType: PostRecoverPassword.MethodEnum, msisdn: string): void => {
    switch (methodType) {
      case PostRecoverPassword.MethodEnum.EMAIL:
        this.router.navigate(['/forgot-password/email/' + msisdn]).then(isRedirectSuccessful => {
      ...
      case PostRecoverPassword.MethodEnum.SMS:
        this.router.navigate(['/forgot-password/pin-code/' + msisdn]).then(isRedirectSuccessful => {
          if (!isRedirectSuccessful) {
            this.alertService.pushDangerAlert(Alerts.SomethingWentWrongWithRedirect);
