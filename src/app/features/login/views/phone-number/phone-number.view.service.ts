  ...
  private handleRegistrationStatus = (
    ...
    switch (status) {
      case GetRegistrationStatus.StatusEnum.REGISTERED:
        this.router.navigate(['/login/password', msisdn]).then(isRedirectSuccessful => {
      ...
      case GetRegistrationStatus.StatusEnum.UNREGISTERED:
        this.router.navigate(['/login/pin-code', msisdn]).then(isRedirectSuccessful => {
          if (!isRedirectSuccessful) {
            this.alertService.pushDangerAlert(Alerts.SomethingWentWrongWithRedirect);
    ...
    if (isBackendError(error)) {
      switch (error.code) {
        case BackendErrors.NotAllowedToLogin:
          this.router.navigate(['/login/limited-access']).then(isRedirectSuccessful => {
            if (!isRedirectSuccessful) {
              this.alertService.pushDangerAlert(Alerts.SomethingWentWrongWithRedirect);
