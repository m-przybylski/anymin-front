...
export class ConsultationStatComponent {
  ...
  public readonly callsMapping = this.buildTraslationMap('CONSULTATION_INFO.CONSULTATION_ROW.CALLS');
  public readonly commentMapping = this.buildTraslationMap('CONSULTATION_INFO.CONSULTATION_ROW.COMMENTS');
   ...
   */
  private buildTraslationMap(token: string): { [kay: string]: string } {
    return {
      '=0': `${token}.none`,
      '=1': `${token}.single`,
      few: `${token}.few`,
