...
import {
  ChangePasswordStatusEnum, ChangePasswordComponentService, ResetPasswordStatusEnum
} from './change-password.component.service';
import {
  Alerts, AlertService, FormUtilsService, LoggerService, LoggerFactory, InputPasswordErrorsEnum
...
export class ChangePasswordComponent implements OnDestroy {
  ...
  @Output()
  public resetPasswordMethodEmitter$: EventEmitter<PostRecoverPassword.MethodEnum> =
    new EventEmitter<PostRecoverPassword.MethodEnum>();
  ...
  constructor(private passwordService: ChangePasswordComponentService,
  ...
  public onResetPassword = (): void => {
    ...
    this.passwordService.resetPassword(this.msisdn)
  ...
  private updateValidationNewPasswordInput = (): void => {
    if (this.arePasswordsTheSame()) {
      this.passwordForm.controls[this.newPasswordControlName]
        .setErrors({[InputSetPasswordErrors.DuplicatedPassword]: true});
    } else {
  ...
  private displayIncorrectPasswordError = (): void => {
    this.passwordForm.controls[this.currentPasswordControlName]
